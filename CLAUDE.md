...
| `ai-readiness-cartography` | 임의 git 저장소가 'AI 코딩 에이전트가 읽고 안전하게 기여할 수 있는 코드베이스'인지를 **결정론적 스코어러(score.py, stdlib-only)로 정량 측정·시각화**하는 도메인 무관 단일 스킬 (100점·9카테고리(E 검증·실행신호 22/D 의존그래프 18/B 문맥품질 15/C 암묵지 12/H 피드백루프 9/A 네비게이션 8/F 신선도 8/I 환경재현 5/G 성과 3) + 2 blocking gate(Reference Integrity·Executable Verification, 실패 시 등급 상한 — 순수 가중합 아닌 gating 집계) → JSON 점수표+HTML 대시보드+ROI 리팩토링 가이드. 루브릭은 2025~2026 1차 근거로 조작화(ORACLE-SWE 2604.07789 실행신호 최상위·LocAgent 2503.09089 기계 판독 의존 그래프·ETH Zurich 2602.11988 보유율≠성능→novelty·USENIX 2025 slopsquatting→E1 gate·RepoMirage 2605.26177 structure-first anchor(god-file=결합도는 defect-prediction 문헌·라인 수 근거 부재)·Factory/Kenogami lowest-as-ceiling). auto/heuristic/manual 라벨·정직성·제안만. **ai-readable-codebase와 상보**(측정 vs 정성 진단→개선). 외부 스킬 v2를 deep-research 5세션 적대 검증으로 v3 리팩토링) |
...
- [`_docs/skills.md`](_docs/skills.md) — 플러그인별 스킬 목록·커맨드·설명 (frontend-harness / harness-generator / git-harness / meta-harness / product-spec-harness / loop-engineering / review-harness / ops-harness / backend-harness / cicd-harness / context-engineering / agent-orchestration / eval-harness / spec-driven-development / ai-readable-codebase / human-agent-teaming / code-as-harness / llm-guardrails-harness / qa-agent-harness / agent-authorization-harness / ai-readiness-cartography / token-efficiency)
