...
| Skill | Command | Description |
|-------|---------|-------------|
| AI-Readiness Cartography | `/ai-readiness-cartography` | 임의 git 저장소가 'AI 코딩 에이전트가 읽고 안전하게 기여할 수 있는 코드베이스'인지를 **측정하고(결정론 스코어러) 개선을 설계·적용하며 승인 게이트 뒤에 구조·본문 코드를 수정하는(멀티 에이전트)** 단일 스킬(**3모드·자유 조합**). ① 측정 모드: score.py 자동 채점 → HTML 대시보드 → ROI 가이드. ② 진단·개선 모드: score.py를 센서로 2축 진단(승인 게이트)→빌드 가드레일→standalone→**계획·개별 승인 뒤 코드 적용(위험 등급 S/M/L·고위험 opt-in·AST/LSP·behavior 센서)**→수용 증명·재측정. ③ 코드 본문 층위 모드: legibility_scan.py census→주석·명명·granularity 다각도 진단→**3게이트 승인 후 단계별 코드 수정**. 발동 시 실행할 모드 부분집합 확정(배타 아님·여러 개면 ①→②→③ 순) |

**단일 스킬 3모드 (측정 / 진단·개선 / 코드 본문 층위)·자유 조합(배타 아님·①→②→③ 순). ②·③은 각각 4 에이전트(모두 `model: "opus"`)를 spawn.** 핵심 자산:

- **`scripts/score.py`** (stdlib only, Python 3.10+) — 100점·9카테고리(E22/D18/B15/C12/H9/A8/F8/I5/G3) + **3 blocking gate**(Gate-1 Reference Integrity·Gate-2 Executable Verification=Auto·상한 AI-Fragile / Gate-3 Architecture Enforcement=Heuristic·개선 모드 전용·상한 AI-Assisted). **gating 집계** — blocking 결함이 등급에 상한을 씌운다(순수 가중합 아님). import 그래프·결합도 god-file·reference integrity 자동 산출. 개선 모드에서 Phase 0 진단 seed·Phase 4 재측정 델타로 재사용.
- **② 개선 모드 5-Phase** (`agents/`) — `accessibility-assessor`(Phase 0: score.py seed 위 2축 진단 + 5밴드 등급 + Gate-3 예비판정) · `guardrail-architect`(Phase 1 **설계**: 빌드 가드레일·의존 방향 물리 강제·피드백 3차원) · `standalone-designer`(Phase 2 **설계**: port/adapter·use-case seed) · **Phase 3 Apply**(오케스트레이터 + `behavior-guard`: 위험 등급 S/M/L 계획 승인→개별 승인 후 적용·이동/리네임=AST/LSP·센서 관측·고위험 opt-in·L은 UNVERIFIED 상한) · `acceptance-verifier`(Phase 4: 수용 증명 + 결정론 델타 위 강제 probe로 Gate-3·등급 재측정·Phase 3 적용본 대상).
- **③ 본문 층위 4 에이전트** (`agents/`) — `comment-auditor`(주석 4분류→C0·C1) · `naming-analyst`(명명 3축→C2) · `structure-cartographer`(구조 후보→C3·opt-in) · `behavior-guard`(개입 클래스별 센서 실행·관측·generator≠checker). census 스캐너 **`scripts/legibility_scan.py`**(stdlib only·등급 없음·7 탐지기), 개입 정본 `references/intervention-catalog.md`·원리 `legibility-principles.md`. B0~B4 3게이트(계획→개별→최종) 승인 후에만 코드 수정·저장소 층 등급 안 만듦(census만)·리네임=AST/LSP 위임·커밋 안 함.
...
> `ai-readiness-cartography`는 **코드베이스 AI 준비도를 측정(결정론 5밴드+3 gate)하고 그 위에서 구조 개선을 설계·적용하며 승인 게이트 뒤에 코드를 수정**하는 단일 스킬(**3모드·자유 조합**)이다. 등급은 단일 5밴드만 쓰고(L1~L5 폐기·`점수/20=L` 선형변환 금지) enforcement 축은 Gate-3로 흡수한다. **내재화 원칙** — 모드 선택 먼저(①②③ 배타 아닌 자유 조합·여러 개면 ①→②→③ 순·score.py는 ①②가 1회 공유·②+③이면 ③의 P4는 ②에 위임) · 결정론 우선·사람 보강 · gating(Gate-1/2→AI-Fragile, Gate-3→AI-Assisted) · 근거 서열=가중치 서열 · 문서 존재≠좋음(보유율 미채점) · god-file=결합도 · A축≠Q축·구조가 프롬프트보다 먼저·빌드가 강제하고 문서가 설명한다(개선 모드) · generator/checker 분리 · auto/heuristic/manual 라벨 · ①은 측정만·②③은 승인 게이트 뒤에만 코드 수정(②=계획·개별·고위험 opt-in, ③=3게이트·커밋 안 함) · 과장 금지. **경계** — 한 기능 구현(`backend-harness`)·상류 핸드오프 검수(`review-harness`)·하네스 자체 진단(`meta-harness`)·전달 파이프라인(`cicd-harness`)·명세 작성(`spec-driven-development`)·컨텍스트 조립(`context-engineering`)·AI 생성물 judge(`eval-harness`)·병렬화·토폴로지(`agent-orchestration`)·완성 코드 리뷰·PR(`frontend/git-harness`)은 범위 밖. 근거: 측정은 ORACLE-SWE(2604.07789)·LocAgent(2503.09089)·ETH Zurich(2602.11988)·USENIX 2025 slopsquatting·RepoMirage(2605.26177)·Factory/Kenogami; 개선 모드는 flex.team 5부작 + arXiv:2505.10443·2602.11481·2306.09896. 출처: 외부 스킬 v2를 v3로 리팩토링(deep-research 5세션)한 뒤 별도 `ai-readable-codebase` 플러그인을 진단·개선 모드로 흡수(v0.2.0)하고, `code-legibility-harness`를 코드 본문 층위 모드 ③으로 흡수(v0.3.0), 모드 게이트를 배타 택일 → **자유 조합**으로 전환(v0.3.1).
...
임의 git 저장소를 **AI 코딩 에이전트가 읽고 안전하게 기여할 수 있는 코드베이스**로 만들기 위해 코드 설계 품질을 **3단계(① 점수화 → ② 쉬운 것부터 섹션별 승인 개선 → ③ 종합 결과)**로 진단·개선하는 도메인 무관 인터랙티브 멀티 에이전트 하네스.

| 스킬 | 명령 | 설명 |
|------|------|------|
| Design Principle Harness | `/design-principle-harness` | 진입 오케스트레이터. Phase 0 스코프 → Step 1 점수화(score.py 두 층위) → Step 2 쉬운 것부터 섹션별 개선(계획→개별 승인, 구조 opt-in) → Step 3 재측정 델타·종합 결과. 매 단계 승인 게이트. |

**score.py 루브릭** — **Tier A 표면 가독성** /60(A1 변수·함수·클래스/컴포넌트명 명료성 28·A2 명명 규약 일관성 12·A3 주석 건강도 20 — 삭제 감점 + comment-gap 후보(침묵 예외·매직 넘버·정규식) report-only로 '왜' 주석 opt-in 추가 seed) · **Tier B 설계 원칙** /40(B1 복잡도/KISS·B2 응집·결합/SRP·B3 중복/DRY·B4 순환 의존·B5 과대추상/DIP·DI·IoC·YAGNI 각 8) = 총점 100 · **Tier C AI-맥락 신호**(report-only·**총점 미포함**: C1 Semantic Markup & a11y=native 시맨틱 vs `<div onClick>`·native 대체가능 `role=`·무-alt·ARIA 볼륨 / C2 Test Legibility=모호·오도 테스트 제목·getByRole vs testid/CSS/xpath 셀렉터). stdlib only·Python 3.10+·측정만(코드 수정 없음)·신뢰 불가 repo 하드닝(symlink·null byte·byte cap·ReDoS 바운드)·test_score.py 31건.

**에이전트 6종(opus)** — [Track A] `code-scorer`(A1 score.py·층위 점수표) / `staged-refiner`(A2 쉬운 것부터 제안·승인 후 적용) / `behavior-guard`(A·B 공용 generator≠checker·행위 센서) / `acceptance-reporter`(A·B 공용 재측정 델타·오귀속 금지) · [Track B] `ai-access-assessor`(B1 ai_access.py·6지표 SCORED/report-only 해석) / `ai-access-improver`(B2 지표별 개선·build enforces·자동 생성 금지).

**v0.4.0 Track B(AI 접근성)** — 사용자 요청으로 코드 품질 외에 **에이전트가 코드베이스를 이해·안전하게 수정하기 쉬운가**를 측정·개선하는 두 번째 큰 틀 추가. 독립 assessor `ai_access.py`(stdlib only)가 6지표 측정 — **M1 의존성 방향 강제**(dependency-cruiser·eslint-boundaries·import-linter·ArchUnit·Nx·TS project refs 존재+CI fail=**SCORED** Gate)·**M2 독립 실행 가능성**(runnability gate+독립 oracle 가드=**SCORED**)·M3 빌드 피드백·M4 모듈 경계 예측 가능성·M5 패턴 일관성·M6 에이전트 가이드(나머지 report-only). SKILL Phase 0 트랙 선택(A/B/A+B·둘 다면 A→B)→트랙별 Step(A1~A3/B1~B3), 각 트랙 하위 지표/섹션별 승인 게이트. **정직성(deep-research 6지표·13에이전트 적대 검증)**: 6지표 중 5개 '에이전트 이득'은 측정된 개입 아니라 인간-SE+첫 원리 추론이라 **SCORED는 기계 검증 사실(M1·M2)뿐**·**build enforces docs explain**(에이전트 가이드 존재≠성능·arXiv:2602.11988·LLM-생성본 5/8 하락→**자동 생성 금지**)·**tool-index≠code-structure**(LocAgent 92.7%·RepoGraph +32.8%는 툴 이득·layout 귀속 금지·ablation BM25>graph)·**유일 측정 레버=독립 oracle**(격리-green 81~100% false-pass·arXiv:2606.26978)·테스트/컴파일 통과를 correctness로 credit 금지(ImpossibleBench 76% exploit)·규칙/모듈/줄 수 가점 금지. 개입(가드레일·가이드)은 승인 게이트 뒤에만·커밋 안 함. references `ai-accessibility-rubric.md`·`ai-accessibility-playbook.md`·`research/ai-accessibility-dossier.md`. test_ai_access.py 21건(SCORED={M1,M2} 불변식).

**references** — `scoring-rubric.md`(Tier A/B/C·배점·confidence·개선 순서·넣지 말 것) · `design-principles.md`(SOLID·응집/결합·복잡도·중복·DI/IoC·DRY/KISS/YAGNI + Tier C 시맨틱 마크업·a11y·테스트 설명 카탈로그·정적 측정 가능/불가 경계) · `improvement-playbook.md`(쉬운 것부터 안전 개선·메커니즘·불변식·Tier C opt-in 절) · `research/`(2023~2026 1차 근거 dossier: evidence-dossier·rubric-design + **semantic-a11y-test-dossier**(+raw-findings), deep-research 다렌즈 sweep→적대 검증).

**v0.2.0 Tier C(AI-맥락 신호)** — 사용자 요청(시맨틱 태그·웹 접근성·테스트 설명으로 AI 코드 맥락 파악)으로 추가. **report-only·총점 미포함**(직접 개입 근거 없음): 시맨틱 HTML 이득은 **모델 역량 조건부**(WorkArena L1 강한 모델 full HTML +11~17.5pp/약한 모델 a11y-tree 우세·저자 동일군이라 독립 삼각검증 아님)·landmark-vs-divsoup 격리 연구 부재 / **ARIA 볼륨은 오류와 상관**(WebAIM 방향성만·고정 배수 금지·상관≠인과·bad ARIA worse than none) / **올바른 테스트 설명 LLM 이해 이득 미미·오도만 해악**(2404.03114·비대칭)·tests-as-oracle overfitting(2511.16858)·오라클은 actual 인코딩(2410.21136)·accessible selector는 refactor-robust(MEDIUM idiom)이나 AI 이해 근거 없음. → 존재/볼륨 가점 금지·오도/모호/native-대체가능/무-alt만 개선(C-a 테스트 제목 삭제/리네임→C-b role 제거/alt 보완→C-c `<div>`→`<button>` 승격→C-d →getByRole 이관·opt-in 행위 센서)·**자동 ARIA/alt/어서션 생성 금지**. 적대 감사 인용 교정(2504.04372 제목·78%≠81% / 2511.16858 제목 / "~10×" drop / 2508.14727 STRONG).

> `design-principle-harness`는 사용자 요청대로 설계 원칙을 **점수화하되**, 그 점수를 **인증이 아니라 개선 우선순위용 진단 지표**로 다룬다(설계 원칙 점수 목표화=Goodhart/reward-hacking). **내재화 원칙** — 결정론 먼저(손채점 금지) · 두 층위(명명·주석=act-first·사람 근거 견고+오도/stale 신호가 LLM에도 해로워 가중↑·단 LLM 리네임 효과 부호 불안정 / 설계 원칙=defer·사람 대상 검증·약한 프록시·타당도 외삽이라 낮은 confidence·진단) · **개입 연구 없어 총점은 안전 기여 인증 아닌 hotspot 지도**(가장 강한 기여 신호=실행 가능·의존성 무결성은 범위 밖) · 두 축 등급(측정≠타당도) · **쉬운 것부터·무거운 SOLID/구조는 opt-in**(사용자 지침 "무거운 작업 추후·쉬운 것 먼저 승인 개선") · 리네임 **AST/LSP 위임** · 중복은 "잘못된 추상화보다 쌀 수 있음" · 구조는 행위 센서 관측+opt-in·**"테스트 통과≠동작 보존"** · **결정 권한(위험은 경고·수용은 사용자·승인된 고위험 변경 조용한 보류 금지·무테스트→행위 센서 승격·UNVERIFIED는 관측 한계 라벨·execute-or-escalate)** · generator≠checker · 표면 편집을 에이전트 성공률/툴그래프 이득으로 **오귀속 금지** · Step 1 측정만·Step 2·3 승인 뒤 수정·**커밋 안 함** · **독립 동작(타 플러그인 참조·의존 없음·스코어러·루브릭·근거 dossier 내재화)**. **경계** — 완성 코드 리뷰/PR/커밋(`frontend/git-harness`) · 상류 산출물 핸드오프 게이트(`review-harness`) · 하네스 자체 진단·생성(`meta-harness`/`harness-generator`) · 한 기능 구현·실행검증(`backend-harness`) · 파일 트리·디렉터리 조직 같은 저장소 레벨 구조는 범위 밖 — **코드 본문(명명·주석·설계 원칙·시맨틱 마크업·테스트 설명)의 설계 품질 점수화 + 쉬운 것부터 승인 개선**에 특화. 근거: deep-research 2023~2026 1차 소스 적대 검증(references/research/).

## meta-prompting-harness

**메타 프롬프팅**(AI에게 입력할 프롬프트를 AI가 최적화해 생성하게 하고, 그 결과 프롬프트를 새 컨텍스트에 주입해 실제 작업을 돌리는 기법)을 4단계 인터랙티브 멀티 에이전트로 수행하는 도메인 무관 하네스. 널리 쓰이는 메타 프롬프팅 실천 방법론을, 각 기법을 `/deep-research`(6각도·25소스·23 confirmed/2 refuted)로 2023~2026 1차 문헌과 대조해 근거 등급을 붙여 재구성했다.
...
**근거 등급(정직성 핵심)** — **STRONG(1차 근거·단 조건부)**: 메타프롬프팅>직접 프롬프팅(Suzgun&Kalai 2401.12954 +17%·OPRO 2309.03409·PromptAgent 2310.16427·**단 모델/과제 조건부·전이 불안정**) / 생성 전 명확화 질문(ClarifyGPT FSE2024 GPT-4 68→81%·Orchid 2604.21505·**단 명확화는 코딩/추론과 분리된 모델 의존 역량이라 명시 유도 필요**·ClarifyCodeBench 2607.00711) / 성공+검증 조건 명시(**Anthropic 공식**·외부 기준 대비·순진한 자기수정 아님) / fresh 컨텍스트 주입(lost-in-the-middle 2307.03172·Chroma context-rot·**방향성 추론**) / 은탄환 아님(Anthropic). **report-only(folklore·근거 약함)**: 실행 환경 맞춤 변환·큰그림→축소 — 1차 근거 생존 못 함(**"실증" 표현 금지**). **REFUTED(미사용)**: "메타프롬프팅=지휘자가 전문가 인스턴스 생성" 프레이밍(0-3)·"LLM 스스로 명확화 불가"(1-2).

> `meta-prompting-harness`의 **내재화 원칙** — 최적화 프롬프트를 **보편 법칙으로 서술 금지**(자기보고·불안정) · **전부 위임 금지**("네가 다 정해줘"는 하네스 의미 상실) · AI가 깎은 프롬프트는 **사용자 도메인 검증 필수**(불안정→사람 안전장치) · report-only를 실증으로 포장 금지 · 프롬프트 품질 **점수화 금지**(Goodhart) · 이 하네스는 **프롬프트 생성만** 하고 실행(새 창 주입·`/goal`·이미지 생성)은 **사용자가** 함 · **커밋 안 함**. **경계**(산출물이 프롬프트가 아니면 범위 밖) — 이미 목표가 있는 컨텍스트 페이로드 조립·압축·리트리벌 · 하네스/스킬 자체 생성·진단·개선 · 기획문서 PRD·사용자 스토리 · 계획/방법론 정렬 인터뷰 · 자율 반복 루프 실행 · 코드 구현/리뷰/커밋 제외 — **다른 도구에 주입할 지시 프롬프트 자체**를 질문 유도·성공조건·환경 맞춤으로 깎아내는 데 특화. 다른 플러그인을 참조하거나 의존하지 않는 독립 플러그인(references 3종·근거 dossier 내재화). 근거: deep-research 2023~2026 1차 소스 적대 검증.

## org-transformation-harness

회사/팀을 **목적조직·AI native** 방향으로 바꾸려는 실무자·리더를 인터뷰해, **현행을 먼저 진단**하고 **목표를 질문으로 함께 구체화**한 뒤 **"무엇부터 시작할지"를 단계별 할 일**로 정리하는 도메인 무관 인터랙티브 멀티 에이전트 하네스. 시드 인터뷰 프롬프트를 메타 프롬프팅 기법으로 깎아낸 뒤 하네스로 승격, `/deep-research`(6각도)로 2023~2026 1차 문헌 적대 검증.

| 스킬 | 명령 | 설명 |
|------|------|------|
| Org Transformation | `/org-transformation` | 진입 오케스트레이터. Phase 0 현행 조직 진단 → 1 목표 구체화 문진(목적조직·스프린트/일하는 방식·AI native) → 2 간극 진단·실행계획 → 3 적대 검증·공유 요약. 첫 행동=현행 진단. 매 Phase 승인 게이트. |

**4 에이전트(opus)** — `org-cartographer`(Phase 0: 조직/팀 구조·일하는 방식·페인·지금 할 수 있는 강점 관측·발명 금지·미확인 표기) / `vision-elicitor`(Phase 1: 목적조직·스프린트/일하는 방식·AI native를 한 번에 한 질문으로 함께 정의·**임의 정의 금지**·목표상태+동인+트레이드오프·충돌 감지) / `gap-planner`(Phase 2: 현재→목표 간극 진단표 + **'지금 바로 시작 가능한 것' 맨 위**로 영향·노력·의존성 우선순위 할 일 + 3~4단계 로드맵·quick-wins·additive-first) / `plan-critic`(Phase 3: 일반론·지어낸 사실·은탄환·우선순위 오류·숨은 가정 적대 반증 후 경영진/팀 공유용 5~7줄 요약).

**references 4종** — `interview-pillars.md`(4기둥 질문뱅크·현재/목표/간극 추출·한 번에 한 질문·충돌 감지) · `transformation-frames.md`(목적조직·AI native·일하는 방식 정의 렌즈·트레이드오프·안티패턴·등급) · `prioritization.md`(영향·노력·의존성·quick-wins·additive-first·로드맵) · `research/README.md`(2023-2026 1차 dossier·등급·REFUTED 교정).

**근거 등급(정직성 핵심)** — **STRONG(1차·단 조건부)**: AI native는 은탄환 아님(DORA 2024/2025 PRIMARY ~5,000명 — AI 도입이 개인 체감 생산성↑이나 **delivery 안정성↓**·이득은 팀 조건(테스트·버전관리·피드백·플랫폼)에 좌우·프로세스 개선은 small batch·robust testing 없이는 delivery 자동 개선 안 함·병목은 코드 생성 아니라 프로덕션 투입·잦은 우선순위 불안정은 성과·웰빙 해침). **MEDIUM(컨센서스)**: 처방 전 현행 진단(Change Management 학술지·단 실패율 인과는 "likely"로 hedged·미측정·진단 과정 자체 under-theorized) / 목적조직(Cagan Transformed·Team Topologies — "단일 정답 없음"·"모델 아니라 시스템/경계/펀딩/팀설계가 제약"·팀은 complete slice of value를 도메인 경계로 오너십·"많은 회사 첫 시도 실패"). **report-only(folklore)**: quick-wins/additive-first(Kotter short-term wins)·우선순위화(RICE/ICE/WSJF·cost of delay)는 실무 프레임이지 측정된 우월성 아님(**"실증" 금지**). **REFUTED(미인용)**: "기업 GenAI 파일럿 95% 실패"(MIT 인용, 표본·정의·방법론 논란으로 널리 반박)는 수치 미인용·방향성만 · DORA 성과 tier는 고정 벤치마크 아님(매년 군집분석 재산출).

> `org-transformation-harness`의 **내재화 원칙** — 첫 행동=**현행 진단**(목표 먼저 그리지 않음) · **목표 임의 정의 금지**(목적조직·AI native를 AI가 단정 안 하고 질문으로 함께 정의·렌즈는 정답 틀 아님) · **현행 먼저·발명 금지**(미관측은 '미확인'·지금 할 수 있는 강점도 관측=보존 대상) · **한 번에 한 질문(짧게)·답변 분기·충돌 감지** · 정직성 가드레일(**은탄환·'N% 개선' 금지**·수치는 출처·등급과 관찰값으로만·**AI 도입 체감≠시스템 성과**·트레이드오프+안티패턴 동반·**제안만**·조직 변경 자동 실행 안 함·**점수화 안 함**) · **quick-wins·additive-first**(잘 되는 현행 보존·되돌리기 쉬운 개입 우선·전면 전환은 명시 승인 시만) · **영향·노력·의존성** 우선순위(신호+이 순서인 이유 1줄·의존성 선행·점수 절대화 금지) · **커밋 안 함**·opt-in 저장(각 파일 개별 확인). **경계**(범주로만 서술·자매 플러그인 이름 비참조) — 명명된 개발 방법론 선택·추천·PRD/스토리 작성·하네스 자체 생성/진단·사람↔AI 협업 토폴로지 설계·작업의 AI 에이전트 병렬화·CI/CD·IaC 구축·코드 구현/리뷰/커밋 제외 — **사람 조직의 목적조직·AI native 전환 실행계획**(현행 진단→목표 구체화→간극→실행 할 일)에 특화. 다른 플러그인을 참조하거나 의존하지 않는 독립 플러그인(references 4종·근거 dossier 내재화). 근거: deep-research 2023~2026 1차 소스 적대 검증(synthesize 중단으로 검증 통과 claim 6블록으로 dossier 직접 구성).
