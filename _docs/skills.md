...
| Skill | Command | Description |
|-------|---------|-------------|
| Product Spec | `/product-spec` | 기획자(PM)용 **기획문서(PRD) + 사용자 스토리** 작성·검증 + **기획안 DoR 평가(생성 전 선행)** 진입 오케스트레이터. 개발 착수 *전* 단계, 도메인 무관 인터랙티브. Phase 0~4 (요구/문제 정의(Discovery) → 기획안 DoR 평가 → PRD 작성 → 사용자 스토리 도출 → 적대적 검증). **기획안을 인자로 주면** 카드 추출 후 생성 전 DoR 평가 → 보강점 반영해 PRD·스토리 생성. 매 Phase 산출물 미리보기 + **사용자 승인 게이트** 후 진행, 1줄 보고 `[Phase N] {핵심결정} — 다음: {다음}. 진행할까요?` |
...
- **Phase 1 기획안 DoR 평가(모드 A 진입질문 필수·생성 전 선행)** — `dor-evaluator`. PRD·스토리 생성 *전*, 모드 A에선 **진입 질문을 반드시 제시**하고(생략·자동 스킵 금지) 사용자 동의(예/모호한 긍정) 시 **원본 기획안**을 착수 준비도(DoR)로 평가 — DoR 게이트 · INVEST 스코어카드(Testable·Independent=0이면 차단) · GWT 완결성(정상/엣지/에러/빈/권한) · 모호성 린트 · 의존성/계약 참조. `# DoR Review 결과` + **보강 체크리스트**(미체크=보강 대상)를 **채팅으로 제시**하고 그 보강점을 이후 PRD·스토리에 반영. 방법론은 `references/dor-review-rubric.md`에 **내재화**(외부 플러그인 참조·의존 없음 — 단독 설치로 독립 동작). 파일 저장은 마무리에서 사용자가 선택할 때만(`product-spec-review.md`). ([references/dor-review-rubric.md])
...
- **Phase 3 사용자 스토리 도출** — `story-writer`. PRD 요구사항→스토리("…로서 …하고 싶다, 왜냐하면 …") + 수용기준(Given/When/Then) + INVEST 자가점검. Phase 1 보강점(음성 경로 등) 반영. ([references/user-story-guide.md])
- **Phase 4 적대적 검증** — `spec-reviewer`. **칭찬형 금지**: 요구↔스토리 추적 매트릭스(완전성) · INVEST 통과 · 수용기준 관찰성 · 목표↔요구↔스토리 일관성 · 모호/판정불가 문장 색출. **리포트는 채팅 표시만(파일 저장 안 함)**, 결함은 additive-first로 PRD·스토리에 반영.
- **마무리(opt-in 저장)** — Phase 1 기획안 DoR 평가 결과는 **PRD·스토리 생성 후 사용자가 저장을 선택할 때만** `product-spec-review.md`로 저장(즉시 저장 안 함).

> `product-spec-harness`는 **개발 전 제품 기획 산출물(PRD·사용자 스토리) 작성 + 생성 전 기획안 DoR 평가**에 특화한 도메인 무관 인터랙티브 하네스다. 5명의 에이전트(`requirements-analyst`, `dor-evaluator`, `prd-writer`, `story-writer`, `spec-reviewer`)를 서브에이전트로 spawn한다(모두 `model: "opus"`; Phase 순차). 내재화 원칙 — 승인 게이트(미리보기 후 쓰기) · 관찰형 수용기준(Given/When/Then) · 요구↔스토리 완전성 추적 · 적대적 검증(파일 저장 안 함) · 기획안 DoR 평가 내재화(생성 전 선행, 저장 opt-in, Honesty Guardrail·2025+ 근거) · additive-first · 한 번에 한 질문. **독립성** — 단독 설치로 동작하며 다른 마켓플레이스 플러그인을 참조·의존하지 않는다(DoR 평가 방법론 내재화). **경계** — 프론트엔드 화면 구현·기술 설계용 PRD·구현 요구사항·코드 작성, 코드/하네스/커밋 작업, *이미 완성된* 상류 산출물을 핸드오프 게이트로 검수만 하는 작업은 범위가 아니다(트리거 충돌 방지를 위해 description·trigger-eval에 명시).
