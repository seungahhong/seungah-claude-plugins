...
**score.py 루브릭** — **Tier A 표면 가독성** /60(A1 변수·함수·클래스/컴포넌트명 명료성 28·A2 명명 규약 일관성 12·A3 주석 건강도 20) · **Tier B 설계 원칙** /40(B1 복잡도/KISS·B2 응집·결합/SRP·B3 중복/DRY·B4 순환 의존·B5 과대추상/DIP·DI·IoC·YAGNI 각 8) = 총점 100 · **Tier C AI-맥락 신호**(report-only·**총점 미포함**: C1 Semantic Markup & a11y=native 시맨틱 vs `<div onClick>`·native 대체가능 `role=`·무-alt·ARIA 볼륨 / C2 Test Legibility=모호·오도 테스트 제목·getByRole vs testid/CSS/xpath 셀렉터). stdlib only·Python 3.10+·측정만(코드 수정 없음)·신뢰 불가 repo 하드닝(symlink·null byte·byte cap·ReDoS 바운드)·test_score.py 28건.

**에이전트 4종(opus)** — `code-scorer`(Step 1 score.py 실행·두 층위 점수표 해석) / `staged-refiner`(Step 2 쉬운 것부터 개선안 제안·승인 후 적용) / `behavior-guard`(Step 2 generator≠checker·변경마다 행위 센서) / `acceptance-reporter`(Step 3 재측정 델타·종합 결과·오귀속 금지).
