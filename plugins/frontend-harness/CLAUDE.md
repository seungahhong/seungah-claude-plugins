...
hooks/                         # PreToolUse·PostToolUse·Stop 훅
  ...
  guard.sh write-guard.sh skill-dedup.sh incremental-lint.sh stop-lint.sh package-changed.sh
...
| 2026-07-05 | 증분 lint 훅 분리 + 심링크 경계 하드닝 | **(1) 재배선** — PostToolUse(Edit\|Write)를 stop-lint.sh에서 신설 `incremental-lint.sh`로 이관(수정 파일 1개만 lint·`--fix` 후 잔여 에러는 요약을 stderr로 내고 exit 2로 모델에 전파), stop-lint.sh는 Stop 전용(전체 diff·best-effort 비차단)으로 정리. **(2) 파서 공용화** — incremental-lint가 inline python3 대신 공용 `lib.sh`(hook_field, jq→python3 폴백)를 사용해 형제 훅과 정합. **(3) 보안: 심링크 경계 하드닝(incremental-lint + stop-lint 양쪽)** — 문자열 경로만 검사하던 기존 방식이 레포 안에 커밋된 `node_modules`/`.bin`/`.bin/<tool>` → 루트 밖 심링크로 프로젝트 밖 바이너리를 실행하던 경계 우회(재현 확인)를 차단: PROJECT_ROOT·후보 `.bin`·도구 바이너리를 모두 물리 경로(`pwd -P`·심링크 체인 해석, 순환 40홉 캡)로 재검증해 루트 내부일 때만, 검증한 바로 그 `$real`을 실행(TOCTOU 제거). 정상 npm/pnpm 상대 심링크·모노레포는 그대로 lint(가장 가까운 `.bin`이 루트 밖이면 상위 in-repo `.bin`으로 폴백). **(4) 요약 정직성** — 실패 요약의 에러 카운트를 대소문자 무시로 통일(`Error:`도 집계)하고 실패 분기 최소 1건 바닥 처리·무출력 시 대체 메시지("에러 0건" 모순 제거). 적대 리뷰 2라운드(파괴+런타임 배터리 22/22)로 검증 |
| 2026-07-07 | 다각도 검토 반영 — 훅 가드 하드닝 + 커맨드 상대 경로 정정 (v1.3.2) | 3갈래(훅 보안·스크립트 보안·설계/구조) 적대 검토 발견 수정 — **(1) guard.sh 멀티라인 우회 차단**(grep은 행 단위 매칭이라 라인 컨티뉴에이션(`\` + 개행)으로 분리된 `git push \\n --force` 류가 모든 규칙을 통과(재현 확인) → 매칭 전 개행·탭 공백 평탄화, blocklist에 안전한 과차단 방향), echo→printf(플래그 소비 제거, package-changed 동일), `git add` 결합 플래그(-Av)·`commit -n`(--amend 오탐 없음 검증)·`checkout -- .` 커버. **(2) write-guard 대소문자 정규화** — macOS APFS/Windows 기본 대소문자 무시 FS에서 `.ENV`/`server.PEM`/`Credentials.json`이 가드를 우회(재현 확인) → basename 소문자화 후 매칭. **(3) skill-dedup** — git 밖 pwd 폴백이 거대 트리 find→타임아웃→조용한 fail-open이 되던 것을 형제 훅과 동일한 `\|\| exit 0`으로 정리 + find -path 글롭 메타문자 이스케이프. **(4) hooks.json matcher 앵커링**(`^(Edit\|Write)$` 등 — NotebookEdit 오발동 제거). **(5) 커맨드 상대 경로 정정** — 커맨드 7종이 플러그인 루트 기준 `./skills/...`·`./commands/...`로 참조해 파일 위치 기준으로 존재하지 않던 경로를 파일 기준(`../skills/...`·`./x.md`)으로 교정(verifier.md references 3건 포함, 링크 체커 재검증). 훅 우회 배터리 16/16 통과 |
