...
- **인수조건(AC)이 주어지면 그것이 곧 동작 목록이다** — PRD의 인수조건(AC-n.m, Given/When/Then)을 입력으로 받으면 AC 1건당 RED 테스트 ≥1개로 전개한다(Given=준비, When=실행, Then=단언). AC ID를 테스트 이름·주석에 남겨 AC↔테스트를 추적 가능하게 한다. 수렴 루프 재호출이면 미충족 AC만 대상으로 한다.
- 핵심 경로와 복잡한 로직에 집중, 모든 엣지 케이스를 다루지 않는다(단, 주어진 AC는 모두 커버)
- 사용자 승인 후 진행
