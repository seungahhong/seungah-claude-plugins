...
│   ├── dor-evaluator.md           # Phase 1 원본 기획안 DoR 평가 (모드 A 진입질문 필수·생성 전 선행, 채팅 제시·즉시 저장 안 함)
...
| 1 | 기획안 DoR 평가 (모드 A·진입질문 필수·생성 전 선행) | dor-evaluator | `# DoR Review 결과` (DoR 게이트 · INVEST 스코어카드 · GWT 완결성 · 모호성 린트 · 의존성) + **보강 체크리스트** — **채팅 제시, 즉시 저장 안 함** | 모드 A에서 진입질문 **필수 제시**(생략 금지)·평가 실행만 예/아니오, 결과를 PRD/스토리에 반영 |
...
> **입력 모드** — 시작 시 1회 판별하고 **결과를 채팅에 1줄로 선언**한다(이 라벨이 모든 Phase 전이의 기준). 경계가 모호하면 **모드 A로 안전 디폴트**(모드 미정인 채 PRD 직행 금지). (A) 기획안 입력: 기획안에서 문제 정의를 추출(Phase 0) → 생성 전 원본 기획안 DoR 평가(Phase 1; 모드 A에선 **진입 질문 제시 필수**, 평가 실행만 동의 시) → 보강점 반영해 PRD·스토리 반드시 생성 → 적대적 검증. (B) 인터뷰: 기획안이 없으면 Phase 1을 건너뛰고(0→2는 모드 B 사양상 정상), 마무리에서 동의 시 생성 산출물을 대상으로 DoR 평가·저장 선택 적용.
