...
3. **계획 전체를 먼저 승인**받은 뒤, **개별 테스트를 하나씩 승인**받으며 순차로 생성·적용·실행하고(unit/integration은 대상 파일 폴더의 `__test__/`에 두고 외부 API는 캡처 응답 mock으로 double, E2E는 선택 시 테스트 경로를 초기에 문의, nightly full-suite도 캡처 mock로 결정론 실행),
