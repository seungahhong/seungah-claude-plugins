...
## 5. 테스트 배치 & mock 규약
- **배치**: 테스트 대상(컴포넌트/유틸/모듈) 파일이 있는 폴더에 `__test__/` 디렉토리를 만들고 그 안에 테스트 파일을 둔다(co-locate). **파일명은 `*.test.*`/`*.spec.*` 접미사**(폴더명 무관하게 러너 기본 glob이 매칭 — Jest `testMatch`·Vitest `include`). **저장소에 이미 co-location 관습이 있으면 그것을 따라 분산 방지**; `__tests__` 복수 폴더 요구 러너는 '확인 필요' 표기.
