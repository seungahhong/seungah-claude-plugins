...
3. 계획표를 이 형식으로 제시한다:
   ...
   | # | AC-ID | tier | 방법론 스위트(태그 토큰 + 셀렉터) | 오라클(기대·불변식) | 테스트 파일(예정) | GWT 요약 | 근거 |
   |---|-------|------|----------------------------------|---------------------|-------------------|----------|------|
...
2-b. **스위트 태그를 코드에 물리적으로 부여**(`references/test-layering-principles.md` §3.5.2):
   ...
   - **카테고리(스위트)의 첫 테스트를 적용할 때**, 그 스위트의 분리 실행 스크립트(`test:smoke`/`test:regression`/`test:nightly`, sanity면 `test:sanity:changed`)를 `package.json`(또는 저장소 관습)에 추가한다. 스크립트 추가도 코드 변경이므로 **게이트 B 대상**이다(자동 편집 금지).
