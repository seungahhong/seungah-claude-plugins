...
- **P4 반복 절차 → Skill, 반복 무시 → hook (③ 장치화).** 같은 표적이 ≥3 누적(needs_attention)이고 내용이 '절차'면 본문 patch 대신 `structural-redesign-required` + **Skill 추출**을 권고한다(사실은 지침에·절차는 Skill에). 내용이 '규칙인데도 압박 하에 반복적으로 무시됨'이면, 더 강한 advisory 규칙을 더하지 말고 **hook/permission으로 전환(deterministic enforcement)** 을 권고한다 — advisory 규칙은 guardrail이 아니다("반복 무시되는 규칙은 hook로 전환"). 보강을 **어디**에 둘지는 [../meta-harness/references/data-capture-criteria.md §4](../meta-harness/references/data-capture-criteria.md)의 메커니즘 매트릭스(CLAUDE.md/Skill/hook/rule)를 따른다. (근거: Agent Skills best-practices, best-practices/steering blog — 출처는 그 문서에 인라인)
