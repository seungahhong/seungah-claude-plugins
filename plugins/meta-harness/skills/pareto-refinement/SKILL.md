...
| **lean prompt** | 본문은 짧을수록 좋다. 추가 줄·결박은 정합 이득이 있을 때만 | 줄 수가 늘었다면, 그만큼의 behavior-alignment 이득이 있는가? Why 없는 MUST/NEVER를 더하지 않았는가? |
...
```json
{
  ...
  "patch_ref": ".claude/experience-store/{run}/{candidate}/patches/2026-06-03-a11y-skill.md",
  "pareto_coords": {
  ...
  "trigger_eval_ref": "plugins/meta-harness/evals/trigger-eval.json#diag-003",
  "risks": [
