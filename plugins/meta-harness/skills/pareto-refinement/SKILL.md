...
- **P4 결정론은 hook/permission, 절차는 Skill, 판단은 본문 (③ 장치화 = 1급 선택).** 진단의 `enforcement_class`를 **메커니즘 선택의 1차 신호**로 쓴다 — `deterministic-enforce`(매번 반드시 일어나야/막혀야) 또는 `deterministic-record`(매번 반드시 기록)면 advisory 본문(CLAUDE.md/Skill/rules)에 더 적지 말고 **hook/permission patch**를 만든다. 이는 ≥3 반복 후의 폴백이 아니라 **본질적으로 결정론이면 첫 발생부터** 적용한다(반복 무시는 advisory로는 부족하다는 *추가* 신호일 뿐). `judgment`면 본문(claude-md/skill-body)·path-scoped rule. 같은 표적이 ≥3 누적이고 내용이 '절차'면 본문 patch 대신 `structural-redesign-required` + **Skill 추출**(사실은 지침에·절차는 Skill에). **정정**: `.claude/rules`는 결정론이 아니라 advisory다 — 결정론 강제는 hook·permission뿐. 보강을 **어디**에 둘지와 어느 lifecycle event인지는 [../meta-harness/references/data-capture-criteria.md §4](../meta-harness/references/data-capture-criteria.md) 결정 절차 + [../meta-harness/references/hook-lifecycle.md](../meta-harness/references/hook-lifecycle.md) §3 상황→event 표를 따른다. (근거: Agent Skills best-practices, hooks-guide/memory/steering blog — 출처는 그 문서들에 인라인)
...
> **C9 — self-report는 정답이 아니라 검증 대상 주장이다.** proposer가 매긴 4축 좌표·trigger_eval 통과율은 **만든 쪽의 자기 보고**다. 그대로 채택하지 말라 — 적용 전 **만든 쪽이 아닌** 단계(오케스트레이터 Phase 5 lightweight validation, 그리고 description patch면 trigger_eval 실제 실행)가 재확인하고, **사람 승인 게이트 통과 후에만** 재사용한다. 미검증 경험을 재사용하면 틀린 교훈이 다음 회차로 번진다(근거: arXiv 2505.16067 — 데이터 적재 기준 문서 근거, 재검증 안 함; 상세 [data-capture-criteria.md §6](../meta-harness/references/data-capture-criteria.md)). 따라서 좌표는 "내가 통과시켰다"가 아니라 "이 좌표를 이렇게 측정했으니 검증해 달라"는 입력이다.
...
## 결정론적 보강 시 — hook_spec 동봉 (필수)

진단의 `enforcement_class`가 `deterministic-*`라 표적을 **hook/permission**으로 잡으면, patch는 본문 문장이 아니라 **장치 사양(hook_spec)** 을 명시해야 한다. [../meta-harness/references/hook-lifecycle.md](../meta-harness/references/hook-lifecycle.md)로 다음을 고정한다.

- **event** — 상황→event 선택 표(§3)에서 고른 lifecycle event(예: 막아야=PreToolUse, 기록=UserPromptSubmit/PostToolUse, 세션시작 주입=SessionStart, 완료게이트=Stop).
- **role/exit** — `record`(exit 0, 비차단) vs `enforce`(exit 2 또는 `permissionDecision:"deny"`). 하드 금지/허용은 hook보다 `permissions.deny/allow`가 견고(우선순위 최상)임을 우선 검토.
- **matcher** — tool-name event면 tool 패턴(`Edit|Write` 등), matcher 미지원 event(UserPromptSubmit/Stop 등)면 생략/`""`.
- **config 위치** — 플러그인 `hooks/hooks.json`(+스크립트) / 프로젝트 `.claude/settings.json`(→ update-config 핸드오프).
- **why** — 왜 advisory 본문이 아니라 이 event의 이 exit인지(trace 근거 인용).

hook_spec 없는 deterministic-* patch는 Phase 5에서 탈락시킨다(어느 event·exit인지 불명한 hook 제안은 검증 불가). **막되 판단이 필요**한 결함은 command hook이 아니라 **prompt/agent type hook**(결정론 트리거 + 모델 판단)으로 제안한다.
...
- **repo-wide(기본):** patch 표적 = 루트 CLAUDE.md + 임의 SKILL.md 만. 그 밖(agents/·commands/·hooks/·plugin.json·플러그인별 CLAUDE.md)은 patch 없이 `change_kind: "scope-escalation"`(plugin 모드 재실행 권고)으로 보고한다. **표적이 `hook`이면 `scope-escalation`에 hook_spec(event·matcher·exit·스크립트 스케치)을 동봉**해 사용자가 plugin 모드로 바로 옮기게 한다. 레포 루트 메타(.claude-plugin/marketplace.json 등)는 `change_kind: "blocked"`(사용자 직접).
- **plugin(opt-in):** 지목 플러그인의 모든 파일이 경계 안 → `hook` 표적도 정상 patch(`hooks/hooks.json` + 스크립트, hook_spec 동봉).
- **`permission`/프로젝트 `rule`(`.claude/settings.json`·`.claude/rules`):** 어느 스코프든 meta-harness가 직접 쓰지 않는다(P2) → `change_kind: "update-config-handoff"`로 구체 spec(권장 permissions 줄·hook 정의 또는 rule 본문+`paths:`)을 담아 보고하고, 적용은 `update-config` 스킬 + 사용자에게 넘긴다.
...
```json
{
  ...
  "enforcement_class": "judgment | deterministic-enforce | deterministic-record",
  "mechanism": "claude-md | skill-body | description | rule | hook | permission — enforcement_class에서 도출한 보강 메커니즘",
  "hook_spec": "deterministic-* + hook/permission 표적일 때만: { event, role(record|enforce), exit(0|2|permissionDecision), matcher, config_location, why }. 그 외 null",
  "change_kind": "additive | destructive | compose | transfer | scope-escalation | update-config-handoff | blocked | structural-redesign-required | skip",
...
- `change_kind`가 `scope-escalation`/`update-config-handoff`/`blocked`/`structural-redesign-required`면 `patch_ref`는 비우고 `rationale`에 사유·권고를 적는다. `scope-escalation`(hook)·`update-config-handoff`는 `hook_spec`(또는 권장 permissions/rule spec)을 채워 사용자가 바로 옮기게 한다.
- `trigger_eval_ref`는 description을 건드릴 때만, `hook_spec`은 deterministic-* + hook/permission일 때만 채운다(그 외 생략/null).
