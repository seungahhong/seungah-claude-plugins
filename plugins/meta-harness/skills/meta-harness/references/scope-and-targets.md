...
- **scope-escalation** → 표적이 agents/·commands/·hooks/·plugin.json·플러그인별 CLAUDE.md. **patch를 만들지 않고** `change_kind:"scope-escalation"`으로 보고하며, "이 표적을 고치려면 plugin 모드로 재실행하라"고 권고한다. 경계 밖을 몰래 고치지 않는 이유: repo-wide 승인 범위를 넘어서기 때문이다.
- **out-of-scope / blocked** → 표적이 레포 루트 메타(`.claude-plugin/marketplace.json` 등 마켓플레이스/레지스트리 메타). `change_kind:"blocked"`로 보고하고 **사용자 직접 수정**을 안내한다. 자동화가 손대면 배포 메타가 깨질 수 있어 의도적으로 막는다.
