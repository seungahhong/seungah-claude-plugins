...
모든 생성 산출물(experience-store · plugin-store · `.claude/_workspace/`)은 **프로젝트 최상단의 `.claude/` 하위**에 둔다 — 파일시스템을 외부 메모리로 쓰는 file-based memory 패턴(근거: context engineering / Agent Skills). 스코프별 기억은 경로로 격리한다(아래). ※ `.claude/`라는 특정 경로는 출처에 verbatim 없는 합리적 컨벤션이고, 메커니즘(파일 기반 외부 메모리)만 출처 근거가 있다.

> **C5 — store-root는 프로젝트 최상단에서 해석하고, `signals/`는 scope-무관 단일 레인이다.** "작업 디렉토리"는 현재 폴더가 아니라 **프로젝트 최상단**(`git rev-parse --show-toplevel`, git 밖이면 cwd)으로 해석한다. 어느 하위 폴더에서 캡처/실행해도 한 곳에 모이게 하기 위함이다(흩어진 기록은 조용히 사라진다).
> 단, `signals/`는 **항상 repo-wide `.claude/experience-store/signals/`** 에 적재한다 — plugin(opt-in) 스코프에서도 마찬가지다. 훅은 캡처 시점에 평가 스코프(repo-wide냐 plugin이냐)를 알 수 없고, 신호 레인은 본래 **회차·스코프 비종속(cross-session)** 이기 때문이다. 따라서 plugin 스코프 회차의 warm-start도 signals는 **repo-wide 레인에서 읽는다**(plugin-store 아래가 아니다). 회차·후보 산출물(`{run}/{candidate}/...`)만 스코프별 store-root로 갈린다. 상세: [data-capture-criteria.md](./data-capture-criteria.md) C5.
...
- `plugin`(opt-in): `{store-root}` = `.claude/plugin-store/{target}/`
  - `.claude/` 아래 plugin 단위로 격리된 스토어를 둔다(작업 디렉토리 외부 메모리). 그 플러그인의 모든 파일이 패치 경계 안이므로, 회차 기억도 플러그인 단위로 묶는다.
...
{store-root}/
  ...
  signals/                            # 능동 캡처 레인 — UserPromptSubmit 훅이 세션 중 적재(cross-session, 회차·스코프 비종속). ★항상 repo-wide .claude/experience-store/signals/ (plugin 스코프에서도 여기). 훅은 캡처 시점에 스코프를 모른다.
    {YYYY-MM-DD}.jsonl                # 그날의 redirect/fix/augment 발화 원형 (append-only, 요약 금지)
    archive/{period}.jsonl.gz         # (선택·수동 권고) 오래된 전부-consumed 신호의 무손실 압축 보관 — 삭제 아님(C6). traces/엔 적용 안 함.
  patches/
...
.claude/_workspace/                            # 회차 휘발물 (스토어와 분리, 다음 새 실행 시 .claude/_workspace_prev/로 이동)
  {run}/
...
```json
{"step":7,"ts":"2026-06-03T14:18:03Z","actor":"user","kind":"redirect","raw":"방금 그 방향 말고 다시 해줘. 왜 그렇게 갔는지 보고 지금 쓰던 스킬 고쳐줘.","active_skill":"<plugin>:<skill>","source":"current-session"}
```
...
### signals/*.jsonl — 능동 캡처 레인 (UserPromptSubmit 훅)

회차(`/meta-harness` 실행)와 무관하게, **세션 진행 중** 사용자의 redirect/수정/보강 발화를 `hooks/self-heal-capture.sh`(UserPromptSubmit)가 **그때그때 원형으로** 적재하는 레인이다. 날짜별 파일에 append-only로 쌓이며, 회차 디렉토리(`{run}/{candidate}/traces/`)와 달리 **회차에 종속되지 않는다** — "지금은 트리거 안 했지만 나중에 healer가 본다"를 가능케 하는 cross-session 기억이다.

한 줄 = 한 발화. 훅이 패턴 매칭(수정/고쳐/다시/틀렸/빼줘/바꿔/빠졌/보강/방향 다시 + 영어)에 걸린 프롬프트만 적재한다. 적재 기준은 [data-capture-criteria.md](./data-capture-criteria.md)(C1~C9)를 따른다.

```json
{"ts":"2026-06-18T12:15:24Z","actor":"user","kind":"fix","strength":"strong","status":"new","source":"hook:UserPromptSubmit","session_id":"s1","cwd":"/abs/work/dir","root":"/abs/project-top","transcript_path":"/abs/transcript.jsonl","raw":"방금 그거 말고 다시 해줘, 왜 그렇게 갔는지 보고 스킬 수정해줘","captured_by":"meta-harness/self-heal-capture"}
```

- `kind` ∈ `redirect|fix|augment`(훅이 발화 키워드로 1차 분류 — 확정 진단 아님, 보강 후보다).
- `strength` ∈ `strong|weak`(C7) — 단순 재실행("다시 빌드/실행")은 `weak`, 실질 교정("틀렸/아니/바꿔/빼줘/빠졌")은 `strong`. **약한 신호도 버리지 않고 등급만 낮춰 적재**하되, warm-start의 nudge·needs_attention 카운트는 **strong만** 센다(헛걸음 방지). 놓친 신호는 흔적 없이 사라지므로 포착은 넓게, 카운트는 좁게.
- `status` ∈ `new|deferred|consumed`(C6) — 캡처 시 `new`. healer가 이번 회차에 끌어와 진단하면 `consumed`(소비 회차 포인터를 `index.json`에 남김), 사용자가 미루면 `deferred`(다음 warm-start에서 기본 건너뜀). **상태는 전이하되 원본 줄은 삭제·수정하지 않는다** — 정정은 새 상태 표기로(append-only ledger 동일 원칙).
- `root`는 C5 프로젝트 최상단 경로, `transcript_path`는 C4 lightweight identifier(immutable transcript라 역추적이 견고). `raw`는 **발화 원문**(요약 금지, 제1 불변식 동일 적용).
- **캡처 전용·비파괴(record-an-event, exit 0)** — 훅은 이 레인에 적재만 한다. 진단·패치·적용은 하지 않는다(그건 healer = `/meta-harness` + 승인 게이트 소관).
- **보관 규칙(C6 — 안 버림 ≠ 무한정; 선택·수동 권고)**: 오래된 날짜별 `signals/*.jsonl`(전부 consumed/deferred)은 **삭제하지 않고 무손실 압축 보관**(예: 분기별 `signals/archive/<period>.jsonl.gz`)을 권고한다. **이는 자동 단계가 아니다** — experience-historian이 Phase 8에서 *archival 후보로 보고만* 하고, 실제 압축은 사람이 적용한다(자동 압축·삭제 없음). 정리 규칙이 없으면 매 warm-start가 같은 신호를 다시 훑어 비용이 증가한다. 압축본은 필요 시 다시 펼쳐 grep할 수 있고 **`signals/`에만 적용(`{run}/{candidate}/traces/`엔 절대 적용 안 함)** 이라 full-trace 보존(C2)과 충돌하지 않는다.

**소비자(healer)**: `/meta-harness` 실행 시 Phase 1 warm-start가 `status:new`인 미소비 `signals/`(기본 strong)를 이번 회차 후보로 끌어오고, Phase 2 trace-capturer가 해당 발화의 `transcript_path`를 역추적해 직전 산출물·active SKILL을 `traces/*.jsonl`로 정규화한다(원형 보존). 즉 signals는 **회차를 시작시키는 입력 신호**이고, traces는 **그 회차의 진단 근거**다. 소비한 신호는 `status:consumed` + `index.json` 소비 회차 포인터로 중복 재진단을 막는다(원본 signals 줄은 ledger로 보존, 삭제하지 않는다).

## .claude/_workspace 휘발 규칙

`.claude/_workspace/`는 **회차 휘발물**이다 — 스토어(영속)와 분리한다.

- 같은 회차의 부분 재실행은 기존 `.claude/_workspace/{run}/`을 이어 쓴다.
- **새 실행을 시작하면 직전 `.claude/_workspace/`를 `.claude/_workspace_prev/`로 이동**한 뒤 새 `.claude/_workspace/`를 연다. 이전 회차의 중간 산출이 새 회차의 진단을 오염시키지 않게 하기 위함이다.
