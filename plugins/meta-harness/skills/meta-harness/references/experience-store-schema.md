...
모든 생성 산출물(experience-store · plugin-store · `.claude/_workspace/`)은 작업 디렉토리의 **`.claude/` 하위**에 둔다 — 파일시스템을 외부 메모리로 쓰는 file-based memory 패턴(근거: context engineering / Agent Skills). 스코프별 기억은 경로로 격리한다(아래). ※ `.claude/`라는 특정 경로는 출처에 verbatim 없는 합리적 컨벤션이고, 메커니즘(작업 디렉토리 파일 기반 외부 메모리)만 출처 근거가 있다.
...
{store-root}/
  ...
  signals/                            # 능동 캡처 레인 — UserPromptSubmit 훅이 세션 중 적재(cross-session, 회차 비종속)
...
한 줄 = 한 발화. 훅이 패턴 매칭(수정/고쳐/다시/틀렸/보강/방향 다시 + 영어)에 걸린 프롬프트만 적재한다.

```json
{"ts":"2026-06-18T12:15:24Z","actor":"user","kind":"fix","source":"hook:UserPromptSubmit","session_id":"s1","cwd":"/abs/work/dir","transcript_path":"/abs/transcript.jsonl","raw":"방금 그거 말고 다시 해줘, 왜 그렇게 갔는지 보고 스킬 수정해줘","captured_by":"meta-harness/self-heal-capture"}
```

- `kind` ∈ `redirect|fix|augment`(훅이 발화 키워드로 1차 분류 — 확정 진단 아님, 보강 후보다).
- `raw`는 **발화 원문**(요약 금지, 제1 불변식 동일 적용). `transcript_path`로 직전 산출물·active 맥락을 역추적할 수 있다.
- **캡처 전용·비파괴** — 훅은 이 레인에 적재만 한다. 진단·패치·적용은 하지 않는다(그건 healer = `/meta-harness` + 승인 게이트 소관).

**소비자(healer)**: `/meta-harness` 실행 시 Phase 1 warm-start가 미소비 `signals/`를 이번 회차 후보로 끌어오고, Phase 2 trace-capturer가 해당 발화의 `transcript_path`를 역추적해 직전 산출물·active SKILL을 `traces/*.jsonl`로 정규화한다(원형 보존). 즉 signals는 **회차를 시작시키는 입력 신호**이고, traces는 **그 회차의 진단 근거**다. 소비한 신호는 `index.json`에 소비 회차를 포인터로 남겨 중복 재진단을 막는다(원본 signals 줄은 ledger로 보존, 삭제하지 않는다).
