...
| 반복적으로 무시되는 규칙 — 규칙이 있는데도 압박 하에 계속 안 지켜짐 | advisory 규칙으로는 부족 → hook/permission으로 **장치화**(③) 후보 | `agent`/`orchestrator`/hook(scope-escalation) |

> P1·P4·P5 신호의 근거: context engineering(최소·고신호) / Agent Skills best-practices. 중복·비대는 `rule-body-cost`만 올리므로 제거·이관이 Pareto 비후퇴 승리다. 단, 삭제·이관 patch도 승인 게이트를 거치고 **안전장치는 제거 표적으로 삼지 않는다(P2)**. 보강을 **어디**에 둘지(CLAUDE.md/Skill/hook/rule)와 "반복 무시 규칙 → hook 전환"은 [../meta-harness/references/data-capture-criteria.md §4](../meta-harness/references/data-capture-criteria.md)의 메커니즘 매트릭스를 따른다(C8).

> kind 후보 전체: `description` | `skill-body` | `agent` | `orchestrator` | `claude-md` | `plugin-metadata` | `global-memory`. `global-memory`(사용자 `~/.claude` 전역 메모리)는 레포 밖이라 항상 `out-of-scope`(사용자 직접) — 그래도 **표적 칸을 둬 신호가 멈추지 않게** 한다(C8: 갈 곳 없는 신호는 멈춘다).
...
```json
{
  ...
  "target": {
    ...
    "kind": "description | skill-body | agent | orchestrator | claude-md | plugin-metadata | global-memory | null",
