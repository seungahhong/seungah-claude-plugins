...
.claude/plugin-store/{target}/{run}/{candidate}/traces/*.jsonl  # plugin(opt-in)
...
| 중복·잉여 규칙 — 제품 기본 동작과 겹치거나 모델이 이미 하는 걸 또 지시(over-prompting) | 그 규칙이 있는 본문/전역 지침 (제거 후보 — P1) | `skill-body` / `claude-md` |
| 전역 지침 비대 — 사실이 아닌 절차가 루트 CLAUDE.md에 누적 | 루트 CLAUDE.md (절차는 Skill로 이관 — P4/P5) | `claude-md` |
| 1차자료 추측 대체 — 실측(예: Figma·계약·스펙)을 읽지 않고 기본값/추측으로 메움 | 그 작업의 SKILL 본문에 "1차자료 확인 단계"가 부재 (`cause_class: primary-source-substitution`) | `skill-body` |
| **결정론적 강제 근거 — 매번 반드시 일어나야/막혀야 하는데 advisory 본문에만 있음** | advisory(CLAUDE.md/Skill/rules)로는 부족 → **첫 발생부터** hook/permission 장치화 후보(`enforcement_class: deterministic-enforce`) | `hook`/`permission`(repo-wide=scope-escalation) |
| 반복적으로 무시되는 규칙 — 규칙이 있는데도 압박 하에 계속 안 지켜짐 | advisory로는 부족하다는 *추가* 신호 → hook/permission **장치화**(③) | `hook`/`permission`(scope-escalation) |

> P1·P4·P5 신호의 근거: context engineering(최소·고신호) / Agent Skills best-practices. 중복·비대는 `rule-body-cost`만 올리므로 제거·이관이 Pareto 비후퇴 승리다. 단, 삭제·이관 patch도 승인 게이트를 거치고 **안전장치는 제거 표적으로 삼지 않는다(P2)**. 보강을 **어디**에 둘지(CLAUDE.md/Skill/hook/permission/rule)는 [../meta-harness/references/data-capture-criteria.md §4](../meta-harness/references/data-capture-criteria.md)의 결정 절차를 따른다(C8): **먼저 enforcement 성격을 분류**(`deterministic-enforce`/`deterministic-record`/`judgment`)하고, deterministic-*이면 hook/permission으로, hook이면 **상황에 맞는 lifecycle event**를 [../meta-harness/references/hook-lifecycle.md](../meta-harness/references/hook-lifecycle.md)로 고른다. **정정**: `.claude/rules`는 결정론이 아니라 advisory(path-scoped 컨텍스트)다 — 결정론 강제는 hook·permission뿐.

> kind 후보 전체: `description` | `skill-body` | `agent` | `orchestrator` | `claude-md` | `plugin-metadata` | `hook` | `permission` | `rule` | `global-memory`. **`hook`**(플러그인 `hooks/`): plugin 모드=in-boundary, repo-wide=scope-escalation(+제안 hook_spec). **`permission`**(`.claude/settings.json` permissions): meta-harness가 직접 안 씀 → `out-of-scope` → **update-config 핸드오프**. **`rule`**(`.claude/rules/*.md` advisory): 프로젝트 레벨이면 repo-wide 경계 밖 → scope-escalation/핸드오프. **`global-memory`**(사용자 `~/.claude` 전역 메모리)는 레포 밖이라 항상 `out-of-scope`(사용자 직접). 패치 불가 표적도 **표적 칸을 둬 신호가 멈추지 않게** 한다(C8: 갈 곳 없는 신호는 멈춘다).

## scope_status 판정 (패치 경계)

표적이 패치 경계 안인지 함께 판정한다.

- `in-boundary` — repo-wide(기본)에서 표적이 **루트 CLAUDE.md 또는 임의 SKILL.md**. plugin(opt-in)이면 지목 플러그인 내 모든 파일(`hook` 포함).
- `scope-escalation` — repo-wide인데 표적이 agents/·commands/·hooks/·plugin.json·플러그인별 CLAUDE.md. 패치 불가, plugin 모드 재실행이 필요하다고 표시. **표적이 `hook`이면** 단순 "불가"가 아니라 **제안 hook_spec(event·matcher·exit·스크립트 스케치)을 notes에 담아** 사용자가 plugin 모드로 바로 옮기게 한다.
- `out-of-scope` — 레포 루트 메타(.claude-plugin/marketplace.json 등)·`global-memory`·**`permission`**(`.claude/settings.json`)·프로젝트 `rule`. 사용자 직접 영역. 단 `permission`/`rule`은 **update-config 핸드오프**(구체 spec 동봉)로, settings.json 직접 수정은 meta-harness가 하지 않는다(P2).
...
```json
{
  "defect_id": "string",
  "target": {
    "path": "string | null — canonical 경로(레포 상대; 같은 파일을 한 이름으로 — C8 이름 통일)",
    "kind": "description | skill-body | agent | orchestrator | claude-md | plugin-metadata | hook | permission | rule | global-memory | null",
    "scope_status": "in-boundary | scope-escalation | out-of-scope"
  },
  "enforcement_class": "deterministic-enforce | deterministic-record | judgment — 근거의 enforcement 성격(data-capture-criteria §4-1). deterministic-*면 표적 kind는 hook/permission으로 라우팅하고 refiner가 lifecycle event를 고른다. judgment면 claude-md/skill-body. 본질적으로 결정론이면 ≥3 반복을 기다리지 않고 첫 발생부터 분류한다",
  "cause_class": "string | null — 원인 분류(예: primary-source-substitution | missing-enforcement | over-prompting | trigger-imprecision). recurring 카운트가 표적+원인으로 묶이게 한다 (C8)",
...
```json
{
  ...
  "target": {
    "path": "plugins/<target-plugin>/skills/a11y/SKILL.md",
    "kind": "description",
  ...
  "evidence": [
    ...
    { "trace_ref": "plugins/<target-plugin>/skills/a11y/SKILL.md:3", "note": "description의 광범위한 '컴포넌트 리뷰' 문구가 보안 요청을 흡수" }
  ],
