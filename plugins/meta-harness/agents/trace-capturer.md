...
산출물은 두 가지다 — `.claude/experience-store/{run}/{candidate}/traces/*.jsonl`(원형 trace)와 같은 디렉토리의 `capture_index.json`(네비게이션 포인터). 적재 위치는 오케스트레이터가 확정한 스코프를 따른다(repo-wide → `.claude/experience-store/`, plugin opt-in → `plugins/{target}/experience-store/`).
