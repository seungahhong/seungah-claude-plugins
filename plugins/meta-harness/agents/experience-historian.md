...
- **needs_attention ≥3 nudge** — recurring-patterns.md 에서 동일 표적(kind+경로)이 누적 3회에 도달하면 `needs_attention: true` 로 표시하고, 이번 회차에 **새로 3에 도달한** 표적을 오케스트레이터에 명시 보고한다. 3회 누적 표적은 본문 patch 가 아니라 structural-redesign 신호다(pareto-refiner 의 거절 기준과 정합).
- **스코프별 store 위치 분기** — repo-wide(기본)면 `.claude/experience-store/`, plugin(opt-in)이면 `plugins/{target}/experience-store/`. 입력의 평가 스코프로 store-root 를 먼저 확정한 뒤 모든 경로를 그 아래로 잡는다. 한 회차의 모든 산출은 같은 store-root 에 들어간다.
...
- **새 실행**: store-root 자체는 영속이다. _workspace 휘발물은 오케스트레이터가 _workspace_prev/ 로 옮기므로 너는 관여하지 않는다 — 너의 대상은 항상 영속 store-root 다.
