...
- **needs_attention ≥3 nudge** — recurring-patterns.md 에서 동일 표적(kind+경로)이 누적 3회에 도달하면 `needs_attention: true` 로 표시하고, 이번 회차에 **새로 3에 도달한** 표적을 오케스트레이터에 명시 보고한다. 3회 누적 표적은 본문 patch 가 아니라 structural-redesign 신호다(pareto-refiner 의 거절 기준과 정합). 누적 내용이 '절차'면 **Skill 추출 후보**로도 함께 보고한다(사실은 지침에·절차는 Skill에 — 근거: Agent Skills best-practices, P4).
