...
8. **같은 표적 3회 누적 → 구조 재설계 권고.** recurring-patterns.md에서 동일 표적의 누적 카운트가 3 이상(needs_attention)이면 본문 patch를 **거절**하고 `change_kind: "structural-redesign-required"`로 보고한다. 같은 곳을 네 번째로 또 덧대는 것은 본문 비용만 늘리고 근본 원인을 안 고친다 — 표적 분할/에이전트 책임 재배치 같은 구조 변경이 필요하다는 신호다.
