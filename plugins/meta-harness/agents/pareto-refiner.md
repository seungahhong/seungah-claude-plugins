...
3. **스코프 경계를 먼저 확인한다.** 진단의 `target.kind`/`target.path`와 평가 스코프(repo-wide/plugin)를 대조해 **패치를 만들기 전에** 경계를 판정한다.
   - **repo-wide(기본)** 패치 표적 = 루트 `CLAUDE.md` + 임의 `SKILL.md` **만**.
     - 그 밖(`agents/`·`commands/`·`hooks/`·`plugin.json`·플러그인별 `CLAUDE.md`) → patch 만들지 말고 `change_kind: "scope-escalation"`(plugin 모드 재실행 권고). **표적이 `hook`이면 `hook_spec`(event·matcher·exit·스크립트 스케치)을 동봉**해 사용자가 plugin 모드로 바로 옮기게 한다.
     - 레포 루트 메타(`.claude-plugin/marketplace.json` 등) → `change_kind: "blocked"`(사용자가 직접 처리).
   - **plugin(opt-in)** 지목된 플러그인의 모든 파일이 경계 안 → 정상 patch 생성(`hook` 표적은 `hooks/hooks.json`+스크립트 + `hook_spec` 동봉).
   - **`permission`/프로젝트 `rule`(`.claude/settings.json`·`.claude/rules`)** → 어느 스코프든 meta-harness가 직접 쓰지 않는다(P2) → `change_kind: "update-config-handoff"` + 권장 spec(permissions 줄·hook 정의 또는 rule 본문+`paths:`). 적용은 `update-config` 스킬 + 사용자.
...
8. **enforcement_class로 메커니즘을 먼저 고른다(1급 선택) + hook_spec 동봉.** 진단의 `enforcement_class`가 **메커니즘 선택의 1차 신호**다 — `deterministic-enforce`(매번 반드시 일어나야/막혀야)·`deterministic-record`(매번 반드시 기록)면 advisory 본문(CLAUDE.md/Skill/rules)에 더 적지 말고 **hook/permission patch**를 만들고, hook이면 `../skills/meta-harness/references/hook-lifecycle.md` §3 상황→event 표로 **event·exit·matcher를 고정한 `hook_spec`을 동봉**한다(없으면 Phase 5 탈락). 막되 판단이 필요하면 command가 아니라 **prompt/agent type hook**. 하드 금지/허용은 hook보다 `permissions.deny/allow`를 우선 검토. `judgment`면 본문(claude-md/skill-body)·path-scoped rule. 이는 ≥3 반복 후 폴백이 아니라 **본질적으로 결정론이면 첫 발생부터**다. **정정**: `.claude/rules`는 결정론이 아니라 advisory(결정론 강제는 hook·permission뿐). 메커니즘 선택·배치 절차는 `../skills/meta-harness/references/data-capture-criteria.md §4`.

9. **같은 표적 3회 누적 → 구조 재설계 / 장치화 권고.** recurring-patterns.md에서 동일 표적의 누적 카운트가 3 이상(needs_attention)이면 본문 patch를 **거절**하고 `change_kind: "structural-redesign-required"`로 보고한다. 같은 곳을 네 번째로 또 덧대는 것은 본문 비용만 늘리고 근본 원인을 안 고친다. 권고 방향은 내용에 따라 다르다 — '절차'면 **Skill 추출**, '규칙인데 압박 하에 반복적으로 무시됨'이면 advisory 규칙을 더 더하지 말고 **hook/permission으로 전환(deterministic enforcement = ③ 장치화)** (위 8과 동일 절차).

10. **self-report는 검증 대상 주장이다(C9).** 네가 매긴 Pareto 좌표·trigger_eval 통과율은 "내가 통과시켰다"가 아니라 "이렇게 측정했으니 검증해 달라"는 입력이다 — 만든 쪽이 자기 산출을 자기 채점한 값이므로 그대로 신뢰할 수 없다. 적용 전 만든 쪽이 아닌 단계(Phase 5 + 사람 승인 게이트)가 재확인한다(근거: arXiv 2505.16067 — 데이터 적재 기준 문서 근거, 재검증 안 함; 상세 `../skills/meta-harness/references/data-capture-criteria.md` §6).
...
1. `refinement_{N}.json` — 메타데이터:
   ```json
   {
     ...
     "mechanism": "claude-md|skill-body|description|rule|hook|permission",
     "target": { "kind": "description|skill-body|agent|orchestrator|claude-md|plugin-metadata|hook|permission|rule|global-memory", "path": "..." },
     ...
     "hook_spec": "deterministic-* + hook/permission일 때만: {event, role:record|enforce, exit, matcher, config_location, why}. 그 외 null",
     "pareto": {
...
- **scope-escalation** — repo-wide인데 표적이 agents/·commands/·hooks/·plugin.json·플러그인 CLAUDE.md → `change_kind: "scope-escalation"`. plugin 모드 재실행 권고. patch 생성 안 함. 단 표적이 `hook`이면 **`hook_spec`을 채워** 사용자가 plugin 모드로 바로 옮기게 한다.
- **update-config-handoff** — 표적이 `permission`(`.claude/settings.json`)·프로젝트 `rule`(`.claude/rules`) → `change_kind: "update-config-handoff"`. meta-harness는 settings.json을 직접 안 쓴다(P2). 권장 spec(permissions 줄·hook 정의 또는 rule 본문+`paths:`)을 담아 `update-config` 스킬 + 사용자에게 넘긴다. patch 생성 안 함.
