...
- 스키마(고정 키):
  ```json
  {
    ...
    "target": {
      "kind": "description|skill-body|agent|orchestrator|claude-md|plugin-metadata|global-memory",
  ...
  ```
- `target.kind`가 description/skill-body면 in-boundary 가능(루트 CLAUDE.md·임의 SKILL.md). agent/orchestrator(commands)/hooks/plugin-metadata는 repo-wide에서 `scope-escalation`(plugin 모드 재실행 권고). 레포 루트 메타(marketplace.json 등)와 `global-memory`(사용자 `~/.claude` 전역 메모리)는 `out-of-scope`(사용자 직접 → 후속 단계에서 blocked). global-memory는 패치 불가지만 **표적 칸을 둬 신호가 멈추지 않게** 한다(C8).
