...
- **experience-store 경로**: raw trace를 조회할 store-root. repo-wide면 `.claude/experience-store/`, plugin이면 `plugins/{target}/experience-store/`.
...
### 출력
- 파일: `_workspace/{run}/{phase}_diagnosis_{N}.json` (causal-diagnosis 스킬 스키마)
- 스키마(고정 키):
  ```json
  {
    "defect_id": "{N}",
    ...
    "target": {
      "kind": "description|skill-body|agent|orchestrator|claude-md|plugin-metadata",
      "path": "표적 파일 절대/레포상대 경로",
      "scope_status": "in-boundary|scope-escalation|out-of-scope"
    },
    "severity": "high|medium|low|unknown",
    "confidence": "high|medium|low",
    "notes": "needs_user_input·출처 confidence(R3) 등 특기사항"
  }
  ```
- `target.kind`가 description/skill-body면 in-boundary 가능(루트 CLAUDE.md·임의 SKILL.md). agent/orchestrator(commands)/hooks/plugin-metadata는 repo-wide에서 `scope-escalation`(plugin 모드 재실행 권고). 레포 루트 메타(marketplace.json 등)는 `out-of-scope`(사용자 직접 → 후속 단계에서 blocked).
