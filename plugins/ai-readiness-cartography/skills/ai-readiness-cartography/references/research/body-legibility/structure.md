...
> 층위: 코드 **본문**의 함수·모듈 granularity(응집·경계·추출). 저장소 층(의존 그래프·디렉토리·CI)은 측정 모드(score.py) 소관 — 본문 층 개입과 분리.
>
...
- 🚫 `"N줄 초과 = 나쁨"` 류 **함수 라인 수 임계값을 점수·게이트로 사용하지 않는다.** 에이전트 정확도로 검증된 임계값이 없다 — 이 플러그인의 측정 모드(score.py) session-4 C8과 **동일 판정**.
