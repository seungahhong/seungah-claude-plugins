...
- **L 켜면 경고**: "구조 리팩터가 에이전트 성공률을 올린다는 직접 측정 근거는 없습니다(추론). LLM 리팩터의 19~35%가 비등가, 그중 ≈21%는 테스트를 통과합니다." + **테스트 스위트 존재 확인**(없으면 L 거부).
...
- **이동·리네임·시그니처 변경(L)**: **AST/LSP 도구에 위임**(문자열 치환 금지) — rope/pyright · ts-morph/tsserver · gopls · rust-analyzer. 도구 없으면 해당 변경을 "제안만"으로 강등.
- **generator≠checker**: 설계 에이전트(guardrail/standalone)가 아니라 [`behavior-guard`](../../agents/behavior-guard.md)가 변경 클래스별 센서(컴파일/빌드·타입체크·참조 완전성·테스트·사람 diff)를 **실행해 관측**한다.
- **"테스트 통과 = 동작 보존" 금지**(비등가 ≈21%가 테스트 통과) — 구조 리팩터(L)는 최선에도 **UNVERIFIED** 상한. 센서가 red면 되돌린다.
