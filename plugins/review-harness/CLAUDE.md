...
└── evals/
    └── trigger-eval.json              # 트리거 경계 평가(자매 플러그인과 reciprocal should-not 가드)
```
...
- **상류 산출물 게이트**: 이 하네스는 *코드가 아니라* 코드 착수 전 상류 산출물을 핸드오프 시점에 검수한다. 완성된 코드 리뷰는 `frontend-harness`(`/review`·qa-inspector·security-audit)가, PRD·스토리 *작성*은 `product-spec-harness`가, 커밋/PR 리뷰는 `git-harness`가 담당한다.
...
- **트리거 가드**: 새 트리거 표현 변경 시 자매 플러그인(`product-spec-harness`/`frontend-harness`)의 경계 케이스와 reciprocal하게 `evals/trigger-eval.json`을 갱신한다.
