...
- 이 하네스는 **코드 본문의 설계 품질**(명명·주석·설계 원칙·시맨틱 마크업·테스트 설명)을 점수화하고 개선한다. 파일 트리 배치·디렉터리 조직·빌드/문서 산출물 같은 **저장소 레벨 구조**나 실행·의존성 무결성은 다루지 않는다(코드 *본문* 층위에 한정).
