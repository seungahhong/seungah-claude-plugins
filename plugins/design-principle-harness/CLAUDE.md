...
- **경계** — 완성 코드 리뷰·PR·커밋, 상류 핸드오프 게이트, 하네스 자체 진단·생성, 한 기능 구현·실행검증, 그리고 파일 트리·디렉터리 조직 같은 저장소 레벨 구조는 범위 밖. 이 하네스는 **코드 본문의 설계 품질(명명·주석·설계 원칙·시맨틱 마크업·테스트 설명)을 점수화하고 쉬운 것부터 승인 개선**하는 데 특화한다.
