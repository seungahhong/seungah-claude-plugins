...
## 평가 스코프 준수 (patch 만들기 전 최우선 체크)

오케스트레이터가 `평가 스코프`(`repo-wide` | `plugin:<plugin>`)를 주입한다. patch를 만들기 전에 진단의 `target.scope_status` 를 먼저 본다.

- `target.scope_status: "plugin-only"` (repo-wide에서 표적이 `agents/`·`commands/`·`hooks/`·`plugin.json`·플러그인별 `CLAUDE.md`) → **patch를 만들지 않는다.** `change_kind: "scope-escalation"`, `rationale: "repo-wide 패치 경계 밖 표적 — 단일 플러그인 평가 모드 재실행 필요"` 로 보고하고 표적 경로는 그대로 남긴다(사용자가 plugin 모드로 재실행할 때 쓴다).
- `target.scope_status: "out-of-scope"` (레포 루트 `.claude-plugin/marketplace.json` 등 어떤 플러그인에도 속하지 않는 메타데이터) → plugin 모드에서도 못 고친다. **patch를 만들지 않고** `change_kind: "blocked"`, `rationale: "레포 루트 메타데이터 — 사용자 직접 수정"`. scope-escalation이 아니다(plugin 모드 재실행도 막다른 길).
- `target.scope_status: "in-boundary"` → 정상 patch 진행.
- 경계 안이라도 표적이 루트 `CLAUDE.md`(`kind: "claude-md"`)면 아래 가이드 F를 따른다.
...
### F. 루트 CLAUDE.md (프로젝트 지침) 수정 — `change_kind: "claude-md"`

- 표적 = 레포 루트 `CLAUDE.md`. 전역 규칙이라 한 줄 수정이 모든 스킬에 파급된다 — 가장 보수적으로.
- 결박형 규칙 추가가 아니라 모호/상충한 컨벤션을 명료화하는 방향. Why를 함께 박는다.
- Risks에 "이 규칙 변경이 영향을 주는 다른 플러그인/스킬"을 반드시 적는다.
...
- `change_kind: "scope-escalation"` — repo-wide 모드에서 표적이 패치 경계 밖(`scope_status: "plugin-only"`). patch 없이 "단일 플러그인 평가 모드 재실행 필요"를 보고. 표적 경로는 남긴다.

이 네 가지 출력은 오케스트레이터가 Phase 4 사용자 게이트에 그대로 노출한다.
...
- **경계 밖인데 patch 강행** — repo-wide에서 `scope_status: "plugin-only"` 인데 `SKILL.md` 로 우회 patch를 만든다. `scope-escalation` 으로 보고해야 한다.
