...
- Phase 1: 대상 = `plugins/frontend-harness/skills/review/SKILL.md`, 결함 = 트리거 누락 1건
- Phase 2-2: `target.kind: "description"`, `evidence: "'코드 한 번 봐줘' → no skill triggered"`
- Phase 3: `change_kind: "description"`, `trigger_eval_1.json` (should-trigger 8 + should-not-trigger 8) 동봉
- Phase 4: 사용자 승인
- Phase 5: `description` 패치 적용
- Phase 7: `plugins/frontend-harness/evolution-memory/history.jsonl` 1줄 추가
...
- Phase 1: 대상 = `plugins/frontend-harness/commands/orchestrator.md`(또는 review.md), 결함 = Phase 4 실패
- Phase 2-2: 5신호 중 #4(Phase 간 스키마 불일치) 가능성 우선 검토. `target.kind: "orchestrator"` 또는 `"skill"`
- Phase 3: 본문/오케스트레이터 Phase 정의 패치 + Risks(다른 Phase 영향) 명시
- Phase 4: 사용자 승인 (수정 영향 큼 → 신중)
...
| 1 | **한 번에 한 질문씩 인터뷰** (대상 / 결함 / trajectory 가용 여부) | `_workspace/{phase}_intake.md` 기록 + 임계치 도달 표적 우선 알림 |
...
| 5 | accepted 만 Edit 적용 | 본문 < 500줄 / frontmatter 유효 / 트리거 충돌 자체 점검 |
| 6 | (description 수정 시) 트리거 충돌 검증 | `harness-generator` / `skill-creator` / 다른 진입 스킬과 키워드 겹침 |
| 7 | 회차 요약 표 노출 | `evolution-memory/history.jsonl` + `recurring-patterns.md` 갱신 |
...
### 영속 메모리 (`{대상 플러그인}/evolution-memory/`)

대상 하네스의 플러그인 루트에 만들어집니다 (예: `plugins/frontend-harness/evolution-memory/`). 회차 간 패턴 누적.
...
└── evals/                                   # 평가 / 테스트 자료
    ├── trigger-eval.json                    # 트리거 평가 큐 16건 (should-trigger 5 + near-miss 9 + 무관 2)
    ├── evals.json                           # Phase dry-run 시나리오 3건 + assertion
    └── dry-run-report.md                    # dry-run 결과 (8/8 assertion PASS)
```
...
- `evals/trigger-eval.json` — 16건의 자연어 쿼리에 대한 expected 트리거 표. skill-creator §"description optimization" 의 should-trigger / should-not-trigger 패턴을 따름.
- `evals/evals.json` — 3개 시나리오(정상/임계치/증거 부족)의 Phase 흐름 예상 경로 + assertion.
- `evals/dry-run-report.md` — `SKILL.md` 본문과 시나리오를 매핑한 dry-run 결과. **8/8 assertion PASS** + 잠재 gap 2건 식별(첫 self-bootstrapping 진화 후보).
