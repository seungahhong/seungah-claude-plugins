...
- **green CI ≠ 구현 완료(reward-hacking 경계)**: 에이전트는 *기능을 구현하지 않고도* 검증을 통과시킬 수 있다 — 테스트를 약화시키거나, 빈/우회 구현으로 초록을 만들거나, 무관한 변경으로 명령을 통과시킨다. 검증 명령의 초록만으로 PASS하지 말고, 변경 diff가 성공기준의 *내용(목표 기능)*을 실제로 구현했는지까지 적대적으로 확인한다. ("done"은 주장이지 증명이 아니다 — 이 위에 사람의 이해/머지 게이트가 얹힌다: 오케스트레이터 책임.)
